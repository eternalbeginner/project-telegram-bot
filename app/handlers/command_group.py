...
async def handler(app: Client, message: Message):
  ...
  try:
    log.debug("Executing handler for group sended message")
    ...
    if message.chat.type not in cmd_module.__visibility__:
      log.warn("Command: {} isn't available on group chat".format(cmd_name))

      await message.reply(
        "Command ini tidak tersedia untuk group chat",
        reply_to_message_id=msg_id
      )
